...
/*
	These values calibrate the ITDB02-2.2 Screen:
	Short dimension (TP_X) min is 468, max is 3850
	Long dimension (TP_Y) min is 493, max is 3800
	Modified Sandy Noble 2012.04.21
*/
#define PixSizeX	19.71
#define PixOffsX	341
#define PixSizeY	15.36
#define PixOffsY	493

/* Mod to make compilable in arduino 1.0 */
...
int ITDB02_Touch::getX()
{
	...
	if (orient == PORTRAIT)
	{
		if (PixSizeX>=0)
		{
			value = 176-((TP_X-PixOffsX)/PixSizeX);
		}
		else
		{
	...
	{
		if (PixSizeY<0)
			value = 220-((TP_Y-PixOffsY)/-PixSizeY);
		else
...
int ITDB02_Touch::getY()
{
	...
	else
	{
		if (PixSizeX>=0)
		{
			value = 1-((TP_X-PixOffsX)/PixSizeX);
			// modified to add display size Sandy Noble 2012.04.21
			value += 176;
		}
		else
		{
